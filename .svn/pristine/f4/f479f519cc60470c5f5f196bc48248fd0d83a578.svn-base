...
namespace LaHerradura
{
    public class CTA_CTE
    {
        public static void asientaPago(List<DAL.CTACTE_EXPENSAS> lst,
            List<DAL.PAGOS_X_FACTURA> lstPagos, DateTime fechaPago, string fechaRecalculo)
        {
            try
            {
                string obs = fechaRecalculo;
                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    ...
                    foreach (var item in lst)
                    {
                        if (item.PAGO_TOTAL)
                        {
                            asientaPagoCompleto(item, nroRecibo, fechaPago, obs);
                        }
                        else
                        {
                            asientaPagoCta(item, fechaPago, nroRecibo, lstPagos, obs);
                        }
                    }
        ...
        public static void asientaPagoCta(DAL.CTACTE_EXPENSAS obj,
            DateTime fechaPago, int nroRecibo, List<DAL.PAGOS_X_FACTURA> lstPagos, string fechaRecalculo)
        {
            try
            {
                ...
                else
                {
                    ...
                    //capital_pagado = 0;
                    int nroCta = obj.NRO_CTA;
                    decimal monto = obj.MONTO_PAGADO;
                    DAL.MOV_BILLETERA objBilletera = new DAL.MOV_BILLETERA();
                    obj.FECHA = LaHerradura.Utils.Utils.getFechaActual();
                    DAL.PERSONAS_X_INMUEBLES objPer =
                        DAL.PERSONAS_X_INMUEBLES.getByNroCta(obj.NRO_CTA);
                    objBilletera.ID_PERSONA = objPer.ID_PERSONA;
                    objBilletera.MONTO = obj.MONTO_PAGADO;
                    objBilletera.NRO_CTA = obj.NRO_CTA;
                    objBilletera.NRO_RECIBO = nroRecibo;
                ...
                if (fechaRecalculo != string.Empty)
                {
                    obs = string.Format("Con intereses recalculados al {0}", fechaRecalculo);
                    objMov2.FECHA_ULTIMO_PAGO = Convert.ToDateTime(fechaRecalculo);
                }
                else
                    objMov2.FECHA_ULTIMO_PAGO = objMov2.FECHA;
                objMov2.OBS = obs;
                DAL.CTACTE_EXPENSAS.asientoPago(objMov2);

                DAL.CTACTE_EXPENSAS objMov1 = new DAL.CTACTE_EXPENSAS();
                objMov1.ID = obj.ID;
                objMov1.PERIODO = obj.PERIODO;
                objMov1.NRO_CTA = obj.NRO_CTA;
                obj.NRO_RECIBO_PAGO = nroRecibo;
                objMov1.PAGADO = false;
                if (fechaRecalculo != string.Empty)
                    objMov1.FECHA_ULTIMO_PAGO = Convert.ToDateTime(fechaRecalculo);
                else
                ...
                DAL.CTACTE_EXPENSAS.recalculo(LaHerradura.Utils.Utils.getFechaActual(), objMov1.PERIODO, objMov1.NRO_CTA, objMov1.ID);
        ...
        public static void asientaPagoCompleto(DAL.CTACTE_EXPENSAS obj,
            int nroRecibo, DateTime fechaPago, string fechaRecalculo)
        {
            try
            {
                decimal capital_pagado = 0;
                ...
                objMov1.ID_MEDIO_PAGO = obj.ID_MEDIO_PAGO;
                DAL.CTACTE_EXPENSAS.asientoPagoCtaMov1(objMov1);
                //DAL.CTACTE_EXPENSAS.recalculo( objMov1.PERIODO, objMov1.NRO_CTA, objMov1.ID);
            }
            catch (Exception ex)
            {
                throw ex;
            ...
            List<DAL.PAGOS_X_FACTURA> lstPagos, DateTime fechaPago)
        {
            try
            {
                ...
                int nroCta = 0;
                int nroRecibo = DAL.CTACTE_EXPENSAS.getMaxNroRecibo() + 1;
                DateTime fecha = LaHerradura.Utils.Utils.getFechaActual();
                using (TransactionScope scope = new TransactionScope())
                {
                    int primerRecibo = nroRecibo;
                    if (lst.Count > 1)
