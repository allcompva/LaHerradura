...
namespace BLL
{
            ...
            try
            {
                ...
                foreach (var item in lst)
                {
                    ...
                    if (pagos == 1)
                    {
                        ...
                        DAL.CTACTE_EXPENSAS.recalculo(DAL.UTILS.getFechaActual(), objMov1.PERIODO, objMov1.NRO_CTA, objMov1.ID);
                        //if (item.TIPO_MOVIMIENTO == 3)
                        //    DAL.CTACTE_EXPENSAS.recalculoPlan(D item.PERIODO, item.NRO_CTA, item.ID, item.NRO_PLAN_PAGO);
                    }
                    else
                    {
                        if (pagos > 1)
                        {
                            ...
                            if (lstPagos[lstPagos.Count - 1] != null)
                            {
                                if (nro_recibo == lstPagos[lstPagos.Count - 1].NRO_RECIBO_PAGO)
                                {
                                    ...
                                    DAL.CTACTE_EXPENSAS.recalculo(UTILS.getFechaActual(), objMov1.PERIODO, objMov1.NRO_CTA, objMov1.ID);
                                }
                                else
                                {
    ...
    DAL.PAGOS_X_FACTURA_GASTOS.read(item.NRO_RECIBO_PAGO);
                        ...
                        if (pagos == 1)
                        {
                            ...
                            //if (item.TIPO_MOVIMIENTO == 3)
                            //    DAL.CTACTE_EXPENSAS.recalculoPlan(, item.PERIODO, item.NRO_CTA, item.ID, item.NRO_PLAN_PAGO);
                        }
                        else
                        {
                            if (pagos > 1)
